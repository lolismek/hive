...
export function TaskCard({ task }: TaskCardProps) {
  ...
  return (
    <Link href={`/task/${task.id}`} className="block group">
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl overflow-hidden hover:shadow-[var(--shadow-elevated)] transition-shadow focus-visible:ring-2 focus-visible:ring-[var(--color-accent)] cursor-pointer h-full flex flex-col">
        {/* Content */}
